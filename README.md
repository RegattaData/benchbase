...
```bash
java -jar benchbase.jar -b tpcc -c config/regatta/sample_tpcc_config.xml --create=true --load=true --execute=true
```
...
The connection and workload properties should be configured via `config/regatta/sample_tpcc_config.xml` and in `sample_chbenchmark_config.xml` for the TPC-C and CH benchmarks.
...
```bash
java -jar benchbase.jar -b tpcc,chbenchmark -c config/regatta/sample_chbenchmark_config.xml --create=true --load=true --execute=true
```
...
```bash
mvn clean compile exec:java -P regatta -Dexec.args="-b tpcc -c config/regatta/sample_tpcc_config.xml --create=true --load=true --execute=true"
```
